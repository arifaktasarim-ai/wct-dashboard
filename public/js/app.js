...
function initLoginForm() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const beniHatirla = document.getElementById('loginBeniHatirla').checked;
    ...
    try {
      const res = await fetch('/api/auth/login', {
        ...
        body: JSON.stringify({ bolumId, kullaniciAdi, sifre, beniHatirla })
      });
...
async function deleteAsdSapmaKayit(id) {
  if (!confirm('Bu ASD/Sapma kaydını silmek istediğinize emin misiniz? (Bağlı aksiyon, Aksiyonlar listesinde kalmaya devam eder; onu silmek isterseniz ayrıca oradan silmeniz gerekir.)')) return;
  const res = await fetch(`/api/asd-sapma/${id}`, { method: 'DELETE' });
  if (!res.ok) {
